  ...
  template: `
    <div class="p-6 md:p-10 max-w-4xl mx-auto animate-fade-in-up">
      ...
      <div class="flex items-center gap-4 mb-8">
        <a routerLink="/dashboard" class="p-2 rounded-lg bg-white border border-slate-200 hover:bg-slate-50 text-slate-500 hover:text-slate-700 transition-colors shadow-sm">
        ...
        <div>
          ...
          <p class="text-slate-500 mt-1">{{ isEditMode ? 'Update' : 'Define' }} constraints and rules appropriately.</p>
        </div>
      </div>

      <div *ngIf="errorMsg" class="bg-red-50 border border-red-100 text-red-600 p-4 rounded-xl text-sm mb-6 flex items-center gap-2">
         <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
         {{ errorMsg }}
      </div>

      <div class="glass-panel p-8 rounded-2xl relative overflow-hidden">
        <div class="absolute top-0 right-0 w-64 h-64 bg-primary-100 rounded-full blur-[80px] pointer-events-none"></div>
        ...
        <form [formGroup]="materialForm" (ngSubmit)="onSubmit()" class="relative z-10">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="group">
                <label class="form-label">Quantity <span class="text-primary-600">*</span></label>
                <input type="number" formControlName="quantity" class="input-field" step="0.01" />
              </div>
              <div class="group">
                <label class="form-label">Unit <span class="text-primary-600">*</span></label>
                <select formControlName="unit_of_measure" class="input-field py-[13px]">
          ...
          <div class="group mb-6">
            <label class="form-label">Lifecycle Status <span class="text-primary-600">*</span></label>
            <div class="flex gap-4">
               <label class="flex items-center gap-2 cursor-pointer p-4 bg-white rounded-xl border border-slate-200 flex-1 hover:border-primary-300 transition-colors text-slate-700" [class.ring-2]="materialForm.get('status')?.value === 'active'" [class.ring-primary-400]="materialForm.get('status')?.value === 'active'">
                 <input type="radio" formControlName="status" value="active" class="hidden" />
                 <span class="w-3 h-3 rounded-full bg-emerald-500"></span> Active Production
               </label>
               <label class="flex items-center gap-2 cursor-pointer p-4 bg-white rounded-xl border border-slate-200 flex-1 hover:border-primary-300 transition-colors text-slate-700" [class.ring-2]="materialForm.get('status')?.value === 'inactive'" [class.ring-primary-400]="materialForm.get('status')?.value === 'inactive'">
                 <input type="radio" formControlName="status" value="inactive" class="hidden" />
                 <span class="w-3 h-3 rounded-full bg-red-500"></span> Inactive Phase
               </label>
            </div>
          </div>
          ...
          <div class="flex justify-end gap-3 pt-6 border-t border-slate-200">
             <a routerLink="/dashboard" class="btn-secondary text-sm">Cancel</a>
             <button type="submit" [disabled]="materialForm.invalid || isLoading" class="btn-primary min-w-[120px] text-sm">
